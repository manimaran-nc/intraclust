...
def test_kpr(cluster,cid='cid',y='y'):
...
def test_keq(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y)    
        pii = yi/ni
        wi = np.repeat(1/k,k)
...
def test_tkeqs(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y)
        pii = yi/ni
        wi = np.repeat(1/k,k)
...
def test_kprs(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y)
        pii = yi/ni
        wi = ni/N
...
def test_stab(cluster,cid='cid',y='y',kappa=0.45):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y)
...
def test_ub(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y)
        n0 = (1/(k-1))*(N - sum(square(ni)/N))
        yisq = square(yi)
...
def test_aov(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y)    
        n0 = (1/(k-1)) * (N - sum(square(ni)/N))
        yisq = square(yi)
...
def test_mod_aov(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y)        
...
def test_fleiss_cuzick(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y) 
        piio = sum(yi)/sum(ni)
...
def test_mak(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y) 
        yisq = square(yi)
...
def test_peq(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y) 
...
def test_pgp(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y) 
        mu_pgp = sum(yi/ni)/k    
...
def test_ppr(cluster,cid='cid',y='y'):
    if check_data(cluster,cid=cid,y=y):
        k,ni,N,yi = df_param(cluster,cid=cid,y=y) 
        mu_ppr = sum(yi)/N
...
def test_aov_conf_interval(rho_aov,zalpha,n0):
...
def test_wald_conf_interval(rho_aov,yi,N,k):
    piio = sum(yi)/N
